...
void LCD_ShowNum(u16 x, u16 y, u32 num, u8 len, u8 size)
{
    ...
    for (t = 0; t < len; t++)
    {//temp由高到低提取num，一直到个位
        temp = (num / LCD_Pow(10, len - t - 1)) % 10;//LCD_Pow等同于pow,temp取0到9
		//找到第一个非0数，比如00012显示为12；如果num是0则直接显示0。
        if (enshow == 0 && t < (len - 1))//还未遍历到num的个位
        {
            if (temp == 0)
            {
            ...
            else enshow = 1;//找到第一个非0数，开始打印数字

        }

        LCD_ShowChar(x + (size / 2)*t, y, temp + '0', size, 0);//temp + '0'	数字转字符
    }
}
...
void LCD_Showcoordinatesystem(u16 xbasepoint,u16 ybasepoint,u16 xstep,u16 ystep,u16 steplength)
{
	int i;
	//u16 dif=3;
	//xbasepoint-=dif;
	...
	LCD_DrawLine(xbasepoint, ybasepoint, xbasepoint+steplength*xstep, ybasepoint); //axis X
	
	LCD_DrawLine(xbasepoint, ybasepoint, xbasepoint, ybasepoint-steplength*ystep); //axis Y
	
//	LCD_DrawLine(xbasepoint+steplength*xstep-10, ybasepoint+7, xbasepoint+steplength*xstep, ybasepoint); //arrow X
//	LCD_DrawLine(xbasepoint+steplength*xstep-10, ybasepoint-7, xbasepoint+steplength*xstep, ybasepoint); //arrow X
//	LCD_DrawLine(xbasepoint+7, ybasepoint-steplength*ystep+10, xbasepoint, ybasepoint-steplength*ystep); //arrow Y
//	LCD_DrawLine(xbasepoint-7, ybasepoint-steplength*ystep+10, xbasepoint, ybasepoint-steplength*ystep); //arrow Y
	//边框加粗
//	LCD_DrawLine(xbasepoint+4, ybasepoint+4, xbasepoint+steplength*xstep+4, ybasepoint-4); //axis X
//	LCD_DrawLine(xbasepoint+4, ybasepoint+4, xbasepoint+steplength*xstep+4, ybasepoint-4); //axis X
//	LCD_DrawLine(xbasepoint, ybasepoint, xbasepoint, ybasepoint-steplength*ystep); //axis Y右移
	
	LCD_ShowNum(xbasepoint-10,ybasepoint+10,80,2,16);  //写零点
	LCD_ShowString(xbasepoint+steplength*xstep+20, ybasepoint+10,32,16,16,(u8*)"MHz"); //写x
	LCD_ShowString(xbasepoint-30, ybasepoint-steplength*ystep-25,32,16,16,(u8*)"dbm"); //写y
	//xbasepoint+=dif;
	for(i=1;i<=xstep;i++)
	{
		if(i==xstep)
			...
			POINT_COLOR=GRAY;
		//LCD_DrawLine(xbasepoint+steplength*i,ybasepoint+5,xbasepoint+steplength*i,ybasepoint-5);//画横轴上的点
		LCD_DrawLine(xbasepoint+steplength*i,ybasepoint,xbasepoint+steplength*i,ybasepoint-steplength*ystep);//画横轴竖线
		POINT_COLOR=WHITE;
		LCD_ShowxNum(xbasepoint+steplength*i-8,ybasepoint+12,80.0+2.0*i,3,16,0);//显示横轴坐标
	}		
	//xbasepoint-=dif;
	for(i=1;i<=ystep;i++)
	{
		if(i==ystep)
			...
			POINT_COLOR=GRAY;
//		LCD_DrawLine(xbasepoint,ybasepoint-steplength*i,xbasepoint+5,ybasepoint-steplength*i);//画纵轴上的点
		LCD_DrawLine(xbasepoint, ybasepoint-steplength*i,xbasepoint+steplength*xstep,ybasepoint-steplength*i);//画纵轴横线
//		LCD_ShowxFloat(xbasepoint-60,ybasepoint-steplength*i-16/2, 80, 1, 3, 16, 0);//显示纵轴坐标，字体高度16，为了居中故y-16/2
		POINT_COLOR=WHITE;
		LCD_ShowxNum(xbasepoint-30,ybasepoint-steplength*i-16/2, 80-10*i, 3, 16, 0);
		LCD_ShowChar(xbasepoint-35,ybasepoint-steplength*i-16/2,'-',16,0);
	}
		LCD_ShowxNum(xbasepoint-30,ybasepoint-16/2, 80, 3, 16, 0);
		LCD_ShowChar(xbasepoint-35,ybasepoint-16/2,'-',16,0);	
//	LCD_ShowNum(10+step,basepoint+10,10,2,16);  //写横轴上的数据
//	LCD_ShowNum(10+step*2,basepoint+10,20,2,16);
//	LCD_ShowNum(10+step*3,basepoint+10,30,2,16);
//  LCD_ShowNum(10+step*4,basepoint+10,40,2,16);		
}
