...
void LCD_DrawLine(u16 x1, u16 y1, u16 x2, u16 y2)
{
    ...
    for (t = 0; t <= distance + 1; t++ )    //画线输出
    {
        LCD_DrawPoint(uRow, uCol); //画点
				LCD_DrawPoint(uRow-1, uCol-1);
				LCD_DrawPoint(uRow-1, uCol);
				LCD_DrawPoint(uRow-1, uCol+1);
...
//起始的菜单的显示
void LCD_Menu(int row, int* mode,int *menuflag, char** menuitem)
{
	int WIDTH = 480;
	int HEIGHT = 800;
	int deltaw=WIDTH/(row+2);
	int i,flag=0;		//flag为是否选定
	char key_char;		//从键盘读到的值
	char ptr_temp,ptr_last;
	ptr_temp=ptr_last=0;
	LCD_Display_Dir(1);
	//显示菜单即六行选项
	for(i=1;i<=row;i++){
		POINT_COLOR=BLACK;
		LCD_ShowString(HEIGHT/3,i*deltaw,272,24,24,(u8*)menuitem[i-1]);
	}
	LCD_Fill(HEIGHT/3-32,(ptr_temp+1)*deltaw,HEIGHT/3-8,(ptr_temp+1)*deltaw+24,GRAY);
	...
	while(1)
	{
		...
		if(key_char=='1')			//up
		{
			if(ptr_temp==0)ptr_temp = row-1;			//如果-1变成5
			else ptr_temp-=1;
			flag=0;
		}
		if(key_char=='2')			//down
		{
			if(ptr_temp==(row-1))ptr_temp = 0;	//如果6则变成0
			else ptr_temp+=1;
			flag=0;
		}
		if(key_char=='3')
		...
		if(key_char=='4'&&flag==1)
		{
			break;
		}
		if(flag)
		{
			LCD_Fill(HEIGHT/3-32,(ptr_temp+1)*deltaw,HEIGHT/3-8,(ptr_temp+1)*deltaw+24,RED);
			*mode=ptr_temp;
		}
		else if(ptr_last!=ptr_temp){
			LCD_Fill(HEIGHT/3-32,(ptr_last+1)*deltaw,HEIGHT/3-8,(ptr_last+1)*deltaw+24,WHITE);
			LCD_Fill(HEIGHT/3-32,(ptr_temp+1)*deltaw,HEIGHT/3-8,(ptr_temp+1)*deltaw+24,GRAY);
		}
	}
	*menuflag=0;
