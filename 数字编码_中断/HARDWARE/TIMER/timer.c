...
u8	detect_flag=1;			//是否进行检波，不用的时候设置为0节省资源
u8	count_flag=0;
long long	count=0;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)//更新中断（溢出）发生时执行
{
	...
	if((TIM5CH1_CAPTURE_STA&0X80)==0)//还未完整捕获一次
	{
...
{
	if(detect_flag)//开启检波	(TIM5CH1_CAPTURE_STA&0X80)==0)//还未完整捕获一次
	{
...
//		}
		if(TIM5CH1_CAPTURE_STA&0X40==0)		//捕获到一个上升沿 		
		{
			if(count_flag==0){//前一个上升沿
				TIM5CH1_CAPTURE_STA=0;			//清空
				TIM5CH1_CAPTURE_VAL=0;
				TIM5CH1_CAPTURE_STA|=0X40;		//标记捕获到了上升沿
				__HAL_TIM_DISABLE(&TIM5_Handler);        //关闭定时器5
				__HAL_TIM_SET_COUNTER(&TIM5_Handler,0);//将TIM5的计数值设置为0，可能需要
				//仍然是上升沿，不需要重设
	//			TIM_RESET_CAPTUREPOLARITY(&TIM5_Handler,TIM_CHANNEL_1);   //一定要先清除原来的设置！！
	//			TIM_SET_CAPTUREPOLARITY(&TIM5_Handler,TIM_CHANNEL_1,TIM_ICPOLARITY_RISING);//定时器5通道1设置为上升沿捕获
				__HAL_TIM_ENABLE(&TIM5_Handler);//使能定时器5，准备捕获下一次上升沿
			}
			else{//后一个上升沿
				count_flag=0;//归零，形成递推
				count=TIM5CH1_CAPTURE_STA&0X3F; 
				count*=0XFFFFFFFF;		 	    //溢出时间总和
				count+=TIM5CH1_CAPTURE_VAL;      //得到总的高电平时间，分频84，所以单位是us
				//TIM5CH1_CAPTURE_STA=0;          //开启下一次捕获，会自动进入下一次捕获的
			}
		}
		else
		{
